...
impl ImagePngBackend {
    ...
    pub fn save_png(&self, path: &std::path::Path) -> Result<(), String> {
        self.image.borrow().save(path).map_err(|e| e.to_string())
    }
    ...
    fn draw_circle_stroke_impl(&self, center: Point2d, radius: f32, stroke: Stroke) {
        ...
        for i in -steps..=steps {
            ...
            if r > 0 {
                draw_hollow_circle_mut(&mut *img, (center.x as i32, center.y as i32), r, color);
            }
        }
    }
...
impl AbstractPainter for ImagePngBackend {
    ...
    fn rich_text(&self, pos: Point2d, align: Align2, spans: &[super::backend::TextSpan]) {
        ...
        let total_width: f32 = spans
            .iter()
            .map(|span| self.calculate_text_width(&span.text, span.font_size))
            .sum();
