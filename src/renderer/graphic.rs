...
impl Drawable for Graphic {
    fn draw(&self, ctx: &RenderContext) {
        ...
        if self.arrow_type.is_some() {
            eprintln!("Drawing Graphic id={}: type={}, arrow_type={:?}, bbox={:?}", 
...
impl Graphic {
    fn draw_line(&self, ctx: &RenderContext) {
        ...
        let (start, end) = if let (Some(head), Some(tail)) = (&self.head_3d, &self.tail_3d) {
            ...
            let start_pos = ctx.cdx_to_screen(&Point2d { x: bbox.left, y: bbox.top });
            let end_pos = ctx.cdx_to_screen(&Point2d { x: bbox.right, y: bbox.bottom });
            
            // Debug: Log arrow coordinates
            if self.arrow_type.is_some() {
                eprintln!("Arrow line_seg: start={:?}, end={:?}, bbox={:?}", start_pos, end_pos, bbox);
            }
    ...
    fn draw_oval(&self, ctx: &RenderContext) {
        ...
        let center = ctx.cdx_to_screen(&Point2d { x: center_x, y: center_y });
    ...
    fn draw_bracket(&self, ctx: &RenderContext) {
        ...
        let top_left = ctx.cdx_to_screen(&Point2d { x: bbox.left, y: bbox.top });
        let bottom_left = ctx.cdx_to_screen(&Point2d { x: bbox.left, y: bbox.bottom });
        let top_right = ctx.cdx_to_screen(&Point2d { x: bbox.right, y: bbox.top });
        let bottom_right = ctx.cdx_to_screen(&Point2d { x: bbox.right, y: bbox.bottom });
        ...
        let lip_size = self.bracket_lip_size.map(|s| s as f32).unwrap_or(height * 0.05);

        // Draw left bracket: [
        ctx.painter.line_segment([top_left, bottom_left], stroke);
        ctx.painter.line_segment([top_left, egui::pos2(top_left.x + lip_size, top_left.y)], stroke);
        ctx.painter.line_segment([bottom_left, egui::pos2(bottom_left.x + lip_size, bottom_left.y)], stroke);

        // Draw right bracket: ]
        ctx.painter.line_segment([top_right, bottom_right], stroke);
        ctx.painter.line_segment([top_right, egui::pos2(top_right.x - lip_size, top_right.y)], stroke);
        ctx.painter.line_segment([bottom_right, egui::pos2(bottom_right.x - lip_size, bottom_right.y)], stroke);
    }

    fn draw_arrowhead(&self, ctx: &RenderContext, _start: egui::Pos2, end: egui::Pos2, color: egui::Color32) {
        ...
        eprintln!("Arrowhead: size={}, arrowhead_size={:?}, scale={}, line_length={}", 
