...
impl TaggedObject for ObjectTag {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        tag.tag_name = raw.get_property(CDXPROP_TAGNAME)
            .and_then(|v| String::from_utf8(v.to_vec()).ok());
        
        tag.z_order = raw.get_property(CDXPROP_ZORDER)
            .and_then(|v| i16::decode(v).ok());
        
        tag.visible = raw.get_property(CDXPROP_VISIBLE)
            .and_then(|v| bool::decode(v).ok());
        
        tag.bounding_box = raw.get_property(CDXPROP_BOUNDINGBOX)
            .and_then(|v| Rectangle::decode(v).ok());
        
        tag.position = raw.get_property(CDXPROP_POSITION)
            .and_then(|v| Point2d::decode(v).ok());
        
        tag.foreground_color = raw.get_property(CDXPROP_FOREGROUNDCOLOR)
            .and_then(|v| u16::decode(v).ok());
        
        tag.tag_type = raw.get_property(CDXPROP_TAGTYPE)
            .and_then(|v| i16::decode(v).ok());
        
        tag.tag_value = raw.get_property(CDXPROP_TAGVALUE)
            .and_then(|v| String::from_utf8(v.to_vec()).ok());
