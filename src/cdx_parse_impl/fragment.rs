...
impl TaggedObject for Fragment {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        let mole_racemic = raw
            .get_property(CDXPROP_MOLE_RACEMIC)
            .and_then(|v| bool::decode(v).ok());
        let mole_absolute = raw
            .get_property(CDXPROP_MOLE_ABSOLUTE)
            .and_then(|v| bool::decode(v).ok());
        let mole_relative = raw
            .get_property(CDXPROP_MOLE_RELATIVE)
            .and_then(|v| bool::decode(v).ok());
        let mole_weight = raw
            .get_property(CDXPROP_MOLE_WEIGHT)
            .and_then(|v| f64::decode(v).ok());
        let frag_connection_order = raw
            .get_property(CDXPROP_FRAG_CONNECTION_ORDER)
