...
impl TaggedObject for ReactionStep {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        step.bounding_box = raw
            .get_property(CDXPROP_BOUNDINGBOX)
            .and_then(|v| Rectangle::decode(v).ok());

        // Parse object ID lists (each ID is u32, 4 bytes)
        step.reaction_step_reactants = raw
            .get_property(CDXPROP_REACTIONSTEP_REACTANTS)
            .map(|v| parse_id_list(v));

        step.reaction_step_products = raw
            .get_property(CDXPROP_REACTIONSTEP_PRODUCTS)
            .map(|v| parse_id_list(v));

        step.reaction_step_plusses = raw
            .get_property(CDXPROP_REACTIONSTEP_PLUSSES)
            .map(|v| parse_id_list(v));

        step.reaction_step_arrows = raw
            .get_property(CDXPROP_REACTIONSTEP_ARROWS)
            .map(|v| parse_id_list(v));

        step.reaction_step_objectsabovearrow = raw
            .get_property(CDXPROP_REACTIONSTEP_OBJECTSABOVEARROW)
            .map(|v| parse_id_list(v));

        step.reaction_step_objectsbelowarrow = raw
            .get_property(CDXPROP_REACTIONSTEP_OBJECTSBELOWARROW)
            .map(|v| parse_id_list(v));
