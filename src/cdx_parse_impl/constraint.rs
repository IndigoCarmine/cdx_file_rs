...
pub const CDXOBJ_CONSTRAINT: u16 = 0x8022;
...
impl TaggedObject for Constraint {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        constraint.z_order = raw.get_property(CDXPROP_ZORDER)
            .and_then(|v| i16::decode(v).ok());
        
        constraint.visible = raw.get_property(CDXPROP_VISIBLE)
            .and_then(|v| bool::decode(v).ok());
        
        constraint.bounding_box = raw.get_property(CDXPROP_BOUNDINGBOX)
            .and_then(|v| Rectangle::decode(v).ok());
        
        constraint.foreground_color = raw.get_property(CDXPROP_FOREGROUNDCOLOR)
