...
impl TaggedObject for Page {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        let background_color = raw
            ...
            .and_then(|v| i16::decode(v).ok());
        let width_pages = raw
            .get_property(CDXPROP_WIDTH_PAGES)
            .and_then(|v| i16::decode(v).ok());
        let height_pages = raw
            .get_property(CDXPROP_HEIGHT_PAGES)
            .and_then(|v| i16::decode(v).ok());
        let drawing_space_type = raw
            .get_property(CDXPROP_DRAWING_SPACE_TYPE)
            .and_then(|v| i8::decode(v).ok());
        let width = raw
            .get_property(CDXPROP_WIDTH)
            .and_then(|v| f64::decode(v).ok());
        let height = raw
            .get_property(CDXPROP_HEIGHT)
            .and_then(|v| f64::decode(v).ok());
        let page_overlap = raw
            .get_property(CDXPROP_PAGE_OVERLAP)
            .and_then(|v| f64::decode(v).ok());
        let header = raw
            .get_property(CDXPROP_HEADER)
            .and_then(|v| CDXString::decode(v).ok());
        let header_position = raw
            .get_property(CDXPROP_HEADER_POSITION)
            .and_then(|v| f64::decode(v).ok());
        let footer = raw
            .get_property(CDXPROP_FOOTER)
            .and_then(|v| CDXString::decode(v).ok());
        let footer_position = raw
            .get_property(CDXPROP_FOOTER_POSITION)
            .and_then(|v| f64::decode(v).ok());
        let print_trim_marks = raw
            .get_property(CDXPROP_PRINT_TRIM_MARKS)
            .and_then(|v| bool::decode(v).ok());
        let splitter_positions = raw
            .get_property(CDXPROP_SPLITTER_POSITIONS)
            .and_then(|v| crate::cdx::binary_codec::decode_u32_array(v).ok());
        let page_definition = raw
            .get_property(CDXPROP_PAGE_DEFINITION)
            .and_then(|v| i8::decode(v).ok());
        let bounds_in_parent = raw
            .get_property(CDXPROP_BOUNDS_IN_PARENT)
            .and_then(|v| crate::cdx::values::Rectangle::decode(v).ok());
