...
impl BinaryCodec for CDXString {
    ...
    fn decode(data: &[u8]) -> Result<Self, CdxError> {
        if data.len() < 2 {
            return Err(CdxError::DecodeError("Not enough bytes for CDXString".to_string()));
        }
        
        let mut cursor = Cursor::new(data);
        
        // Read style run count
        let run_count = cursor.read_u16::<LittleEndian>()
            .map_err(|e| CdxError::DecodeError(e.to_string()))? as usize;
        
        // Read style runs
        let mut style_runs = Vec::with_capacity(run_count);
        for _ in 0..run_count {
            let char_index = cursor.read_u16::<LittleEndian>()
                .map_err(|e| CdxError::DecodeError(e.to_string()))?;
            let font_index = cursor.read_u16::<LittleEndian>()
                .map_err(|e| CdxError::DecodeError(e.to_string()))?;
            let font_face = cursor.read_u16::<LittleEndian>()
                .map_err(|e| CdxError::DecodeError(e.to_string()))?;
            let font_size = cursor.read_u16::<LittleEndian>()
                .map_err(|e| CdxError::DecodeError(e.to_string()))?;
            let color_index = cursor.read_u16::<LittleEndian>()
                .map_err(|e| CdxError::DecodeError(e.to_string()))?;
