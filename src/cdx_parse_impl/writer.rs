...
impl<W: Write + Seek> CdxWriter<W> {
    ...
    fn write_object(&mut self, obj: &RawCdxObject) -> Result<()> {
        self.writer.write_le(&obj.tag).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        self.writer.write_le(&obj.id).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        ...
        self.writer.write_le(&0u16).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        Ok(())
    }

    fn write_property(&mut self, prop: &RawCdxProperty) -> Result<()> {
        self.writer.write_le(&prop.tag).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        let data = &prop.value;
        if data.len() >= 0xFFFF {
            self.writer.write_le(&0xFFFFu16).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
            self.writer.write_le(&(data.len() as u32)).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        } else {
            self.writer.write_le(&(data.len() as u16)).map_err(|e| Error::new(std::io::ErrorKind::Other, e))?;
        }
