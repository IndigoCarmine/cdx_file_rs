...
impl TaggedObject for TLCPlate {
    ...
    fn from_raw(raw: RawCdxObject) -> Result<Self, CdxError> {
        ...
        let top_left = raw
            .get_property(CDXPROP_TOP_LEFT)
            .and_then(|v| crate::cdx::values::Point2d::decode(v).ok());
        let top_right = raw
            .get_property(CDXPROP_TOP_RIGHT)
            .and_then(|v| crate::cdx::values::Point2d::decode(v).ok());
        let bottom_right = raw
            .get_property(CDXPROP_BOTTOM_RIGHT)
            .and_then(|v| crate::cdx::values::Point2d::decode(v).ok());
        let bottom_left = raw
            .get_property(CDXPROP_BOTTOM_LEFT)
            .and_then(|v| crate::cdx::values::Point2d::decode(v).ok());
        ...
        let label_face = raw
            ...
            .and_then(|v| i16::decode(v).ok());
        let tlc_origin_fraction = raw
            .get_property(CDXPROP_TLC_ORIGIN_FRACTION)
            .and_then(|v| f64::decode(v).ok());
        let tlc_solvent_front_fraction = raw
            .get_property(CDXPROP_TLC_SOLVENT_FRONT_FRACTION)
            .and_then(|v| f64::decode(v).ok());
        let tlc_show_origin = raw
            .get_property(CDXPROP_TLC_SHOW_ORIGIN)
            .and_then(|v| bool::decode(v).ok());
        let tlc_show_solvent_front = raw
            .get_property(CDXPROP_TLC_SHOW_SOLVENT_FRONT)
            .and_then(|v| bool::decode(v).ok());
        let tlc_show_borders = raw
            .get_property(CDXPROP_TLC_SHOW_BORDERS)
            .and_then(|v| bool::decode(v).ok());
