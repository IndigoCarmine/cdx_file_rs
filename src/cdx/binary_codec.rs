...
impl BinaryCodec for Point2d {
    ...
    fn decode(data: &[u8]) -> Result<Self, CdxError> {
        if data.len() < 8 {
            return Err(CdxError::DecodeError(format!(
                "Not enough bytes for Point2d: got {}, need 8",
                data.len()
            )));
        }
        let mut cursor = Cursor::new(data);
...
impl BinaryCodec for Point3d {
    fn encode(&self) -> Result<Vec<u8>, CdxError> {
        ...
        buf.write_f64::<LittleEndian>(self.x)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        buf.write_f64::<LittleEndian>(self.y)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        buf.write_f64::<LittleEndian>(self.z)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        Ok(buf)
    }
    fn decode(data: &[u8]) -> Result<Self, CdxError> {
        if data.len() < 24 {
            return Err(CdxError::DecodeError(
                "Not enough bytes for Point3d".to_string(),
            ));
        }
        let mut cursor = Cursor::new(data);
        let x = cursor
...
impl BinaryCodec for Rectangle {
    fn encode(&self) -> Result<Vec<u8>, CdxError> {
        ...
        buf.write_f64::<LittleEndian>(self.left)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        buf.write_f64::<LittleEndian>(self.top)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        buf.write_f64::<LittleEndian>(self.right)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        buf.write_f64::<LittleEndian>(self.bottom)
            .map_err(|e| CdxError::DecodeError(e.to_string()))?;
        Ok(buf)
    }
    fn decode(data: &[u8]) -> Result<Self, CdxError> {
        ...
        match data.len() {
            ...
            _ => Err(CdxError::DecodeError(format!(
                "Invalid Rectangle size: got {}, expected 16 or 32 bytes",
                data.len()
            ))),
        }
    }
