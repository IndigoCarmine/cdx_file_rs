...
/// Fallback bond length in CDX coordinates (used when Document has no BondLength)
...
pub struct BondMode {
    ...
    /// Node being dragged (for snap-to-node on drag end)
...
impl BondMode {
    ...
    fn create_bond_from_node(&self, ctx: &ModeContext, source_id: u32) -> Option<(u32, u32)> {
        ...
        println!(
            "Creating bond from node {} at angle {:.2}° (rad: {:.4})",
            source_id,
            angle.to_degrees(),
            angle
        );
        ...
        let bond_length = self.get_default_bond_length(ctx);
        let new_pos = Point2d {
            x: source_pos.x + bond_length * angle.cos(),
            y: source_pos.y + bond_length * angle.sin(),
        };
        println!(
            "Source pos: ({:.2}, {:.2}), New pos: ({:.2}, {:.2})",
            source_pos.x, source_pos.y, new_pos.x, new_pos.y
        );

        // Check if there's an existing node at the calculated position
        let new_pos_screen = ctx.cdx_to_screen(&new_pos);
        let target_node_id =
            if let Some(existing_id) = ctx.hit_test_node(new_pos_screen, NODE_HIT_RADIUS) {
                if existing_id != source_id {
                    println!(
                        "Found existing node {} at target position, connecting to it",
                        existing_id
                    );
                    existing_id
                } else {
                    // Target overlaps with source, create new node anyway
                    let new_id = self.create_node_near(ctx, new_pos, source_id)?;
                    println!("Created new node with id {}", new_id);
                    new_id
                }
            } else {
        ...
        // Create the bond
        let bond_id = self.create_bond(ctx, source_id, target_node_id)?;
        println!(
            "Created bond {} from {} to {}",
            bond_id, source_id, target_node_id
        );

        Some((target_node_id, bond_id))
    }
...
impl ModeHandler for BondMode {
    fn handle_click(&mut self, ctx: &mut ModeContext) {
        ...
        if let Some(node_id) = ctx.hit_test_node(ctx.mouse_pos, NODE_HIT_RADIUS) {
            if self.is_creating_bond {
                ...
                println!(
                    "Clicked on node {} - attempting to create 120° bond",
                    node_id
                );
                if let Some((new_node_id, bond_id)) = self.create_bond_from_node(ctx, node_id) {
                    println!(
                        "Created node {} and bond {} at 120° from node {}",
                        new_node_id, bond_id, node_id
                    );
                } else {
        ...
        if self.is_creating_bond {
            if let Some(source_id) = self.source_node {
                // Check if there's a node near the click position
                if let Some(target_id) = ctx.hit_test_node(ctx.mouse_pos, NODE_HIT_RADIUS) {
                    if target_id != source_id {
                        ...
                        if let Some(bond_id) = self.create_bond(ctx, source_id, target_id) {
                            println!(
                                "Connected to existing node {} with bond {}",
                                target_id, bond_id
                            );
                        }
                    }
    ...
    fn handle_drag(&mut self, ctx: &mut ModeContext) {
        if self.is_creating_bond {
            ...
            ctx.hit_test_node(ctx.mouse_pos - ctx.drag_delta, NODE_HIT_RADIUS)
        {
            // Dragging a node - move it and track it
            self.move_node(ctx, node_id, ctx.drag_delta);
            self.dragged_node = Some(node_id);
        } else if let Some(bond_id) =
            ctx.hit_test_bond(ctx.mouse_pos - ctx.drag_delta, BOND_HIT_THRESHOLD)
        {
    ...
    fn handle_drag_end(&mut self, ctx: &mut ModeContext) {
                ...
                if let Some(target_id) = ctx.hit_test_node(ctx.mouse_pos, NODE_HIT_RADIUS) {
                    if source_id != target_id {
                        ...
                        if let Some(bond_id) = self.create_bond(ctx, source_id, target_id) {
                            println!(
                                "Created bond {} (drag): {} -> {}",
                                bond_id, source_id, target_id
                            );
                        }
                    }
                } else {
                    // No node directly under cursor, but check if new position would overlap with existing node
                    let cdx_pos = ctx.screen_to_cdx(ctx.mouse_pos);
                    let new_pos_screen = ctx.cdx_to_screen(&cdx_pos);
                    if let Some(nearby_id) = ctx.hit_test_node(new_pos_screen, NODE_HIT_RADIUS) {
                        if nearby_id != source_id {
                            // Connect to nearby existing node
                            if let Some(bond_id) = self.create_bond(ctx, source_id, nearby_id) {
                                println!(
                                    "Connected to nearby node {} with bond {} (drag)",
                                    nearby_id, bond_id
                                );
                            }
                        }
        ...
        } else if let Some(dragged_id) = self.dragged_node {
            // Check if dragged node ended up overlapping another node
            if let Some(dragged_pos) = ctx.node_positions.get(&dragged_id) {
                let dragged_screen = ctx.cdx_to_screen(dragged_pos);
                // Look for another node at this position (excluding the dragged node itself)
                for (other_id, other_pos) in ctx.node_positions.iter() {
                    if *other_id != dragged_id {
                        let other_screen = ctx.cdx_to_screen(other_pos);
                        let distance = dragged_screen.distance(other_screen);
                        if distance <= NODE_HIT_RADIUS * ctx.zoom {
                            // Create bond between dragged node and overlapping node
                            if let Some(bond_id) = self.create_bond(ctx, dragged_id, *other_id) {
                                println!(
                                    "Created bond {} between dragged node {} and overlapping node {}",
                                    bond_id, dragged_id, other_id
                                );
                            }
                            break;
                        }
                ...
                if let Some(source_pos) = ctx.node_positions.get(&source_id) {
                    ...
                    if let Some(preview_end) = self.preview_end {
                        ...
                        let stroke = egui::Stroke::new(
                            2.0,
                            egui::Color32::from_rgba_unmultiplied(80, 160, 255, 180),
                        );
    ...
    fn handle_key(&mut self, ctx: &mut ModeContext, key: egui::Key) -> bool {
        match key {
            ...
            egui::Key::K if ctx.ui.input(|i| i.modifiers.command) => {
                ...
                let mut cdx_borrow = ctx.cdx_file.borrow_mut();
                if let Some(cdx_file) = cdx_borrow.as_mut() {
                    super::cleanup::cleanup_structure_selected(cdx_file, ctx.selected_ids);
