    ...
    fn search_node_at_position(
        ...
        for child in node.children() {
            if let Some(child_info) =
                self.search_node_at_position(child, cdx_x, cdx_y, closest_distance)
            {
    ...
    fn create_object_info(
        ...
        match payload {
            NodePayload::Node(node_obj) => {
                let mut details = vec![("Element ID".to_string(), format!("{}", node_obj.id))];
            ...
            NodePayload::TextObject(text_obj) => {
                let text_preview = text_obj
                    .text
                    .as_ref()
                    .map(|t| t.text.chars().take(30).collect::<String>())
                    .unwrap_or_else(|| "".to_string());
                let has_more = text_obj
                    .text
                    .as_ref()
                    .map(|t| t.text.len() > 30)
                    .unwrap_or(false);
                let mut details = vec![(
                    "Text".to_string(),
                    format!("\"{}{}\"", text_preview, if has_more { "..." } else { "" }),
                )];
            ...
            NodePayload::Fragment(frag_obj) => {
                let mut details = vec![("Fragment ID".to_string(), format!("{}", frag_obj.id))];
            ...
            NodePayload::Page(page_obj) => {
                let mut details = vec![("Page ID".to_string(), format!("{}", page_obj.id))];
            ...
            NodePayload::Document(_) => Some(ObjectInfo {
    ...
    fn handle_hover(&self, ctx: &ModeContext, painter: &egui::Painter) {
        ...
        if let Some(ref hovered_info) = hovered {
            ...
            painter.rect_filled(
                rect,
                5.0,
                egui::Color32::from_rgba_unmultiplied(30, 30, 40, 240),
            );
